...
with tabs[3]:
    ...
    if label1 != "Healthy" and 'mask' in locals() and mask is not None:
        mid = data.shape[2] // 2
        seg_fig, seg_ax = plt.subplots(1, 3, figsize=(8, 3))
        seg_ax[0].imshow(data[:, :, mid], cmap="gray"); seg_ax[0].set_title("Original")
        seg_ax[1].imshow(mask[:, :, mid], cmap="gray"); seg_ax[1].set_title("Predicted Mask")
        seg_ax[2].imshow(data[:, :, mid], cmap="gray"); seg_ax[2].imshow(mask[:, :, mid], cmap="Reds", alpha=0.4)
        seg_ax[2].set_title("Overlay")
        for a in seg_ax: a.axis("off")
        seg_overlay_path = os.path.join(tmp_dir, "segmentation_overlay.png")
        plt.tight_layout(); seg_fig.savefig(seg_overlay_path, dpi=150)
        plt.close(seg_fig)

    if label1 != "Healthy" and 'df' in locals() and not df.empty:
        top10 = df.head(10)
        chart_fig, chart_ax = plt.subplots(figsize=(6, 3))
        chart_ax.barh(top10.index[::-1], top10["Percent (%)"][::-1])
        chart_ax.set_xlabel("Percent of Region Affected (%)")
        chart_ax.set_ylabel("Brain Region")
        chart_ax.set_title("Top 10 Most Affected Regions")
        plt.tight_layout()
        region_chart_path = os.path.join(tmp_dir, "region_chart.png")
        chart_fig.savefig(region_chart_path, dpi=150)
        plt.close(chart_fig)

    overlay_img_path = os.path.join(tmp_dir, "overlay.png") if os.path.exists(os.path.join(tmp_dir, "overlay.png")) else None

    doc = SimpleDocTemplate(pdf_path, pagesize=A4)
    story = []
    styles = getSampleStyleSheet()

    story.append(Paragraph("<b>🧠 Brain MRI Diagnostic Report</b>", styles["Title"]))
    story.append(Spacer(1, 12))
    story.append(Paragraph(f"<b>Date:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles["Normal"]))
    story.append(Spacer(1, 12))
    story.append(Paragraph(f"<b>Stage 1 Result:</b> {label1}", styles["Normal"]))
    if label1 != "Healthy":
        story.append(Paragraph(f"<b>Stage 2 Result:</b> {label2}", styles["Normal"]))
    story.append(Spacer(1, 12))

    if seg_overlay_path:
        story.append(Paragraph("<b>Segmentation Results:</b>", styles["Heading2"]))
        story.append(RLImage(seg_overlay_path, width=400, height=150))
        story.append(Spacer(1, 12))

    if label1 != "Healthy" and 'df' in locals() and not df.empty:
        story.append(Paragraph("<b>Top 10 Affected Brain Regions:</b>", styles["Heading2"]))
        top10 = df.head(10)
        data_table = [["Region", "Voxels", "Percent (%)"]] + [[idx, f"{row['Voxels']:.0f}", f"{row['Percent (%)']:.2f}"] for idx, row in top10.iterrows()]
        ...
        if region_chart_path:
            story.append(RLImage(region_chart_path, width=400, height=200))
            story.append(Spacer(1, 12))

        if overlay_img_path:
            story.append(Paragraph("<b>3D Overlay of Affected Regions:</b>", styles["Heading2"]))
            story.append(RLImage(overlay_img_path, width=400, height=200))
            story.append(Spacer(1, 12))

    story.append(Paragraph("This report is AI-generated and should be reviewed by a certified radiologist.", styles["Italic"]))
    doc.build(story)

    with open(pdf_path, "rb") as f:
        st.download_button("📥 Download Full Diagnostic Report", f, file_name=report_name, mime="application/pdf")
